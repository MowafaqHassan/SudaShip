
<template>
  
  <div class="flex items-center justify-center h-screen px-6 bg-gray-200">
    <div class="w-full max-w-sm p-6 bg-white rounded-md shadow-md">
      <div class="flex items-center justify-center">
        <svg
          class="w-8 h-8 ml-2 text-blue-500"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
            d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"
          ></path>
        </svg>
        <span class="text-2xl font-semibold text-gray-700">SudaShip</span>
      </div>

      <form class="mt-4" @submit.prevent="login">
        <label class="block">
          <span class="text-sm text-gray-700">Email</span>
          <input
            type="email"
            class="
              block
              w-full
              mt-1
              border-gray-200
              rounded-md
              focus:border-indigo-600
              focus:ring
              focus:ring-opacity-40
              focus:ring-indigo-500
            "
            v-model="email"
          />
        </label>

        <label class="block mt-3">
          <span class="text-sm text-gray-700">Password</span>
          <input
            type="password"
            class="
              block
              w-full
              mt-1
              border-gray-200
              rounded-md
              focus:border-indigo-600
              focus:ring
              focus:ring-opacity-40
              focus:ring-indigo-500
            "
            v-model="password"
          />
        </label>

        <div class="flex items-center justify-between mt-4">
          <div>
            <label class="inline-flex items-center">
       <router-link to="/Registration">
              <a
              class="block text-sm text-indigo-700 fontme hover:underline"
              href="#"
              >Registeration</a></router-link>
                      </label>
          </div>

          <div>
            <router-link to="/Forget">
            <a
              class="block text-sm text-indigo-700 fontme hover:underline"
              
              >Forgot your password?</a
            ></router-link>
          </div>
        </div>

        <div class="mt-6">
          <button
            type="submit"
            class="
              w-full
              px-4
              py-2
              text-sm text-center text-white
              bg-indigo-600
              rounded-md
              focus:outline-none
              hover:bg-indigo-500
            "
          >
            Sign in
          </button>
        </div>
      </form>
    </div>
  </div>
</template>

<script setup lang="ts">
import { getRedirectRoleRoute } from "@/helpers/routeHelper";
import { ref } from "vue";
import { useRoute, useRouter } from "vue-router";
import {useUserStore} from "../store/user";



const router = useRouter();
const routing = useRoute();
const userStore = useUserStore();
const userRole  = routing.query.role;
const userEmail  = routing.query.email;
const token  = routing.query.token;
  console.log('role is ' , userRole);

  if (userRole == 'administrator') {
    const user={
    email:userEmail,
    role: 'admin',
    token : token
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }else if (userRole == 'headSales') {
    const user={
    email:userEmail,
    role: 'saleman',
    token : token
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }else if (userRole == 'shippingRepresentative') {
    const user={
    email:userEmail,
    role: 'representive',
    token : token
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }else if (userRole == 'shippingAgent') {
    const user={
    email:userEmail,
    role: 'agent',
    token : token 
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }else if (userRole == 'warehouseManager') {
    const user={
    email:userEmail,
    role: 'wherehouse',
    token : token
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }else if (userRole == 'customer') {
    const user={
    email:userEmail,
    role: 'user',
    token : token
  };
  userStore.setUser(user);
    const routerValue =  getRedirectRoleRoute(user.role);
    console.log('routerValue' , routerValue );
    router.push(routerValue);
  }



const email = ref("johndoe@mail.com");
const password = ref("@#!@#asdf1231!_!@#");


const users=[
  {
    email:"Admin@gmail.com",
    role:'admin',
  },
  {
    email:"saleman@gmail.com",
    role:'saleman',
  },
  {
    email:"WhereHouse@gmail.com",
    role:"wherehouse"
  },
  {
    email:"Agent@gmail.com",
    role:"agent"
  },
  {
    email:"Repersentive@gmai.com",
    role:"representive"
  },
  {
    email:"user@gmail.com",
    role:"user"
  }
]
function login() { 
 

const user=search(email.value,users);
  userStore.setUser(user);
  const route = getRedirectRoleRoute(userStore.user.role);
  router.push(route);
}
function search(nameKey: any, myArray: string|any[]){
    for (let i=0; i < myArray.length; i++) {
        if (myArray[i].email  === nameKey) {
            return myArray[i];
        }
    }
}
</script>
